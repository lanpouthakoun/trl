...
class VLLMGeneration:
    ...
    def sync_weights(self):
        ...
        logger.info(
            "sync_weights: model=%s, is_peft=%s, is_fsdp=%s, mode=%s",
            type(model).__name__, is_peft_model(model), is_fsdp_enabled, self.mode,
        )
        ...
        if self.mode == "server" and accelerator.is_main_process:
            if getattr(unwrapped, "_intervention_config", None) is not None:
                ...
                for name, buf in unwrapped.named_buffers():
                    ...
                    if any(f"layers.{i}." in name for i in (0, 12, 23)):
                        logger.info(
                            "[ReFT TRAIN buffer] %s → %s  first4=%s",
                            name, vllm_name, buf.data.flatten()[:4].tolist(),
                        )
